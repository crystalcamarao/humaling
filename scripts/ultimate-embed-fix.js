...
function fixUltimateEmbedsInFile(filePath) {
  let content = fs.readFileSync(filePath, 'utf8');
  
  // Convert any Instagram URL to embed (more comprehensive pattern)
  ...
  // Convert any YouTube URL to embed (more comprehensive pattern)
  content = content.replace(
  ...
  // Convert any Vimeo URL to embed (more comprehensive pattern)
  content = content.replace(
  ...
  console.log(`Fixed ultimate embeds in: ${path.basename(filePath)}`);
}
...
function fixAllBlogPosts() {
  ...
  console.log('🔄 Ultimate embed fix for blog posts...');
  
  files.forEach(file => {
    const filePath = path.join(blogDir, file);
    fixUltimateEmbedsInFile(filePath);
  });
  
  console.log('✅ Ultimate embed fix completed!');
}
